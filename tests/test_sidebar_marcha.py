...
def test_sidebar_muestra_divisoria_y_desfile():
    ...
    assert md.count('class="huellas-march') == 4  # der, izq, der, izq
    assert md.count('class="huellas-march inv"') == 2
...
def test_header_sin_corte_con_sidebar_abierto():
    ...
    assert 'stAppViewContainer"]:has(section[data-testid="stSidebar"][aria-expanded="true"])' in SRC
